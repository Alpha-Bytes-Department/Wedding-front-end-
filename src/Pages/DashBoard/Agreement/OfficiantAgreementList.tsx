...
const OfficiantAgreementList: React.FC = () => {
  ...
  const fetchAgreements = async () => {
    try {
      setLoading(true);
      const response = await axios.get("/agreements/officiant/my");
  ...
  const handleDelete = async (
    ...
    if (protectedStatuses.includes(agreement.status)) {
      GlassSwal.error(
        "Cannot Delete",
        "This agreement cannot be deleted because payment has been completed.",
      );
      return;
    }
    ...
    const result = await GlassSwal.confirm(
      ...
      `Are you sure you want to delete the agreement for ${
        agreement.partner1Name && agreement.partner2Name
          ? `${agreement.partner1Name} & ${agreement.partner2Name}`
          : agreement.userName || "this client"
    ...
    try {
      await axios.delete(`/agreements/${agreement._id}`);
      GlassSwal.success("Deleted", "Agreement has been deleted successfully.");
      fetchAgreements();
    } catch (error: any) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Manage all your ceremony agreements
          </p>
        </div>
        ...
        {agreements.length === 0 ? (
          <div className="bg-white rounded-xl shadow-lg p-12 text-center">
            ...
            <p className="text-gray-500 mb-6">
              Create agreements from the Bookings page when you're ready.
            </p>
            <button
              onClick={() => navigate("/dashboard/bookings")}
              className="bg-gradient-to-r from-orange-400 to-yellow-400 text-white px-6 py-2 rounded-lg font-semibold hover:from-orange-500 hover:to-yellow-500 transition-all"
            >
              Go to Bookings
            </button>
          </div>
            ...
            {agreements.map((agreement) => {
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    navigate(
                      `/dashboard/officiant-agreement?agreementId=${agreement._id}`,
                    )
                  }
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {canDelete(agreement.status) && (
                        <button
                          onClick={(e) => handleDelete(e, agreement)}
...
export default OfficiantAgreementList;
